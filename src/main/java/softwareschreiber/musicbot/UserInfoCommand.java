...
public class UserInfoCommand implements MessageCreateListener {
	...
	public void onMessageCreate(MessageCreateEvent event) {
		...
		author.asUser().ifPresent(user -> {
			...
			embed.addInlineField("Activities", user.getActivities().isEmpty()
					? "none"
		...
		event.getMessage().getServer().ifPresent(server -> embed.addInlineField("Server Admin", author.isServerAdmin() ? "yes" : "no"));
